...
function setupCreateListing() {
  ...
  form.addEventListener("submit", function (event) {
    ...
    postForm("../../server/create-plant.php", { name: form.elements.name.value, species: form.elements.species.value, care_notes: form.elements.care_notes.value })
      ...
      .then(function (sitResponse) { if (!sitResponse.success) throw new Error(sitResponse.error || "Could not create listing"); setMessage(message, "Passport created and sit listed. It is now open for a sitter to claim.", "success"); form.reset(); start.min = today; end.min = today; })
...
function claimSit(event) {
  ...
  postForm("../../server/claim-sit.php", { sit_id: button.dataset.sitId }).then(function (response) { if (!response.success) throw new Error(response.error || "Could not claim this sit"); button.closest("article").remove(); message.textContent = "Sit claimed."; message.className = "status-message form-success"; if (!document.querySelector(".listing-card")) document.getElementById("listing-grid").innerHTML = '<div class="empty-state">No open sits remain.</div>'; }).catch(function (error) { button.disabled = false; message.textContent = error.message; message.className = "status-message form-error"; });
}
...
function setupFindListings() {
  ...
  fetch("../../server/find-listing.php?" + params.toString()).then(function (response) { return response.json(); }).then(function (response) {
    if (!response.success) throw new Error(response.error || "Could not load listings"); if (!response.data.length) { grid.innerHTML = '<div class="empty-state">No plants need a sitter right now.</div>'; return; }
